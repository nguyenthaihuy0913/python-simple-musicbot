...
Bot phát nhạc Discord này được thiết kế với cấu trúc mã nguồn tinh gọn và tối giản nhất. Mục tiêu của dự án là giúp các bạn sinh viên (đặc biệt là thế hệ F21) có thể dễ dàng tiếp cận, đọc hiểu cách ứng dụng Python giao tiếp với API, xử lý luồng sự kiện bất đồng bộ (async/await) và truyền tải dữ liệu âm thanh (audio streaming) theo thời gian thực.

## 🚀 Tính năng cơ bản
- `!play <tên bài hát/link>`: Tự động tham gia kênh thoại, tìm kiếm và phát nhạc trực tiếp từ YouTube.
...
```bash
git clone [https://github.com/TÊN_GITHUB_CỦA_BẠN/techaway-basic-musicbot.git](https://github.com/TÊN_GITHUB_CỦA_BẠN/techaway-basic-musicbot.git)
cd techaway-basic-musicbot
...
Mở file main.py bằng bất kỳ trình soạn thảo code nào (VS Code, Sublime Text,...).
